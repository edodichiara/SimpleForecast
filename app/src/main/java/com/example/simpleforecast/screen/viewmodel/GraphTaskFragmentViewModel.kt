...
@HiltViewModel
class GraphTaskFragmentViewModel @Inject constructor(private val repository: HourlyRepository) : ViewModel() {

    fun retrieveHourlyForecast(){
        CoroutineScope(Dispatchers.Main).launch {
            try {
                val list = repository.getHourlyList()
                Log.d("GraphTaskFragmentViewModel call:", "retrieveHourlyForecast: $list")
            } catch (e: Exception){
                e.printStackTrace()
            }
        }
